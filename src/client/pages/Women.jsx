...
function Women() {
    ...
    const womenCategory = categories.find(
        (cat) => cat.name?.toLowerCase() === "women's perfume" ||
                 cat.name?.toLowerCase() === "women's" ||
                 cat.name?.toLowerCase() === "women perfumes"
    );

    // Filter products for women's category
    const womenProducts = products.filter(
        (product) => product.category_id === womenCategory?.id
    );
        ...
        <div className="bg-white min-h-screen pt-28 pb-20">
            <div className="max-w-7xl mx-auto px-6">
                ...
                <div className="text-center mb-12">
                    <span className="text-xs tracking-[0.2em] text-gray-400 uppercase">For Her</span>
                    <h1 className="text-4xl md:text-5xl font-light text-black mt-2 mb-4">
                        Women's <span className="font-bold">Collection</span>
                    </h1>
                    <div className="w-12 h-px bg-black mx-auto"></div>
                    <p className="text-gray-500 max-w-2xl mx-auto mt-6 text-sm leading-relaxed">
                        Discover our elegant fragrances crafted for the modern woman.
                    </p>
                </div>
                ...
                <div className="mb-8 pb-4 border-b border-gray-100">
                    <p className="text-sm text-gray-500">
                        Showing <span className="text-black font-medium">{womenProducts.length}</span> products
                    </p>
                </div>
